...
postStackMachineR = do
    ...
    let all = Request.all stackMachineRequest
...
execute ADD (x:y:xs) program programCounter jumpCounter =
...
execute SUBTRACT (x:y:xs) program programCounter jumpCounter
